...
`computeSolarRiseSetFast` 中的 Fast 指太阳升落的近似种子和少次迭代策略，不代表 `Accuracy.fast`；该专用路径使用固定太阳模型，不提供 accuracy 参数。

太阳专用链与通用链采用不同的视位置、观测点旋转及太阳半径常量，因此两者结果不要求逐秒相同。需要全部升落和中天事件时，应使用通用接口。

默认站点气压为 1013.25 mbar、温度为 15°C；独立折射函数默认使用 1010 mbar、10°C。
折射模型在 −1°以下截断为零；通用升落求解会拒绝截断跳变造成的伪根。太阳快速回退使用两小时采样，不保证与通用求解器具有相同的擦边事件检出能力。
