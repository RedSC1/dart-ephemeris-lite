...
已完成 JS 1.0.0-rc.1 主包公共功能移植，并同步增加了算术回历接口与后续日月依赖拆分。根入口 200 个导出均有 Dart 对应，编译清单为 `tool/api_surface_check.dart`；Dart 调用形式见 [API 对照表](api-map.md)。
...
- 新版 bazi_core、ziwei_core 已完成底层接入回归；它们保持独立发布，本次先发布天文内核，再分别固定依赖。旧版应用与旧数据迁移仍需单独验证。
...
- 本次发布版本为 `1.0.0-beta.1`，须通过发布内容审查、静态检查、测试与 `dart pub publish --dry-run`；发布不代表所有已知精度限制已消除。
