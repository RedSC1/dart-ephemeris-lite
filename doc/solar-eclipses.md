...
五点 Newton 插值共享日月三维视向量；中心前后 0.25 天内插值，窗口外重新精确求值。影锥与地表采用 WGS84 椭球，地方食采用三维视圆面交叠。没有新增精度开关，保留上游完整算法。
...
全球接触点、最大食地点和瞬时带宽是数值结果；本库不绘制地图，也不导出已删除的旧可变计算器。历史／远期的地理点和地方时刻仍受 ΔT 不确定性影响。测试是 JS/Dart 对拍，不是新增观测精度认证。
